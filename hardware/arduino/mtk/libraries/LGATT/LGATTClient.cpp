...
boolean LGATTClient::getServiceInfo(int index, LGATTUUID& serviceUUID, boolean &isPrimary)
{
    ...
    vm_bt_uuid_with_length_t *tmp = (vm_bt_uuid_with_length_t *)&(cb_data.search_result.uuid[index].uuid.uuid);
    serviceUUID = *tmp;
                                    ...
                                    )
{
    ...
    if (_return_value)
    {
        characteristicUUID = *(vm_bt_uuid_with_length_t*)(&(this->cb_data.characteristic.ch.ch_uuid.uuid));
                          ...
                          )
{
    ...
    if (_return_value)
    {
        descriptorUUID = *(vm_bt_uuid_with_length_t*)(&(this->cb_data.descriptor.descr.descr_uuid.uuid));
    }
